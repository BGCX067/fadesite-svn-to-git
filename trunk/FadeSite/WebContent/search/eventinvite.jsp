...
<%
String contentPage = "/search/eventinvite.jsp";

if (request.getParameter("fromMasterPage") == null)
{
%>
<jsp:forward page="../masterpage.jsp">
	<jsp:param value="Send Event Invite" name="pageTitle"/>
...
<center>
	<h3>Send Event Invite</h3>
</center>
<form method='POST' autocomplete='false' action="<%= "dispatcher?section=search&action=eventinvite&mode=save&userId=" + searchUser.getUserId() %>" style='height: 300px;'>
	<div class="profileBox" style='width: 50%; margin-left:25%; height:100px; align: center;'>
		<table style='float:left; margin-top: 30px;'>
			...
			<tr>
				...
				<td>
					<% List<EventEntity> events = EventDAO.listInvitableEvents(user.getUserId(), searchUser.getUserId()); %>
					<select name="eventId">
						<% for (int i = 0 ; i < events.size() ; i++)
						{%>
							<option value='<%= events.get(i).getEventID() %>'><%= events.get(i).getTitle() %></option>
