...
<%
String contentPage = "/admin/bancreate.jsp";

if (request.getParameter("fromMasterPage") == null)
{
%>
<jsp:forward page="../masterpage.jsp">
	<jsp:param name="pageTitle" value="Set User Ban" />
...
<div style='margin-left: 20px'>
	<form method='POST' autocomplete='off' name="frmBan" action="dispatcher">
		...
		<% UserEntity banUser = (UserEntity) request.getAttribute("banuser"); %>
		
		<input type="hidden" name="section" value="admin"/>
		<input type="hidden" name="action" value="bancreate"/>
		<input type="hidden" name="mode" value="saveban"/>
		<input type="hidden" name="banuserid" value='<%= banUser.getUserId() %>'/>
		...
		<table>
			...
			<tr>
				...
				<td>
					<input type='text' class='datepicker' name='banstart'></input>
			...
			<tr>
				...
				<td>
					<input type='text' class='datepicker' name='banend'></input>
					<label class="reqText">*</label>
				</td>
			</tr>
			<tr>
				...
				<td valign="top">
					<textarea style='width: 400px; height: 150px;' name="reason"></textarea>
					<label class="reqText">*</label>
				</td>
			</tr>
			<tr>
				...
				<% if (request.getAttribute("referrer") != null && request.getAttribute("referrer").toString().compareTo("search") == 0)
					...
					<% } %>
					<input type='button' value='Submit' onclick="return CheckForm()"></input>
				</td>
			</tr>
		</table>
	</form>
