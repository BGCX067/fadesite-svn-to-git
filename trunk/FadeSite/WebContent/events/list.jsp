...
%>
<%
String contentPage = "/events/list.jsp";
...
<div class='subMenu'>
	...
	<a href='dispatcher?section=events&action=manage'>Manage My Event</a> |
	<% } %>
	<a href='dispatcher?section=events&action=personal'>View My Events</a>
</div>
<% List<EventEntity> events = EventDAO.listPublicEventMembers(); %>
	...
	<% for (int i = 0 ; i < events.size() ; i++)
	   {
		...
		%>
			<div style='width: 90%; margin: 30px; margin-bottom:75px;'>
				...
				<% UserEntity updateUser = UserDAO.GetUser(event.getUpdatedByUser()); %>
					<i>Last Updated <label><%= event.getUpdateDateString() %> <%= "by " + updateUser.getFirstname() + " " + updateUser.getSurname() %></label></i> &nbsp;|&nbsp;
					<a href='<%= "dispatcher?section=events&action=view&eventId=" + event.getEventID() %>'>Detailed View</a> |
				<% if (event.getUpdatedByUser() == user.getUserId() || user.getUserTypeId() > 2)
				   {%>
